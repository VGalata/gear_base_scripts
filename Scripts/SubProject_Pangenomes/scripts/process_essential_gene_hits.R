...
hits_sum <- aggregate(rep(1,nrow(hits)), by=list(query_name=hits[,'query_name'], sample=hits$sample), FUN=sum)
write('Aggregated hit counts', log_file, append=TRUE)

# Table: Best hit per query and sample
# Ref: http://stackoverflow.com/questions/24070714/extract-row-corresponding-to-minimum-value-of-a-variable-by-group
# hits_best <- as.data.frame(hits_dt[, .SD[which.min(full_sequence_Evalue)], by=list(sample, query_name)])
hits_best <- as.data.frame(hits_dt[hits_dt[ , .I[which.min(full_sequence_Evalue)], by=list(sample, query_name)]$V1,])
write.table(x=hits_best, file=sprintf('%s_best_hits.tsv', args$obname), sep='\t', row.names=FALSE, quote=FALSE)
write('Selected and saved best hits for each sample/query', log_file, append=TRUE)

# Table: Samples x essential genes = number of hits
...
# add missing queries (e.g. no hits for all samples)
for(q_n in annot$Name){
...
min_prs    <- 99
# save counts as table and core as list
# write.table(x=ov_g_s, file=sprintf('%s_num_hits.tsv', args$obname), sep='\t', quote=FALSE)
write(x=names(ov_g_s_prs[ov_g_s_prs >= min_prs]), file=sprintf('%s_core.txt', args$obname), ncolumns=1)
...
# Table: species x essential genes = mean
